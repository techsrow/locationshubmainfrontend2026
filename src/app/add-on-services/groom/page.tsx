...
export default function BrideAddOnPage() {
  const [images, setImages] = useClientMemoryState<BrideImage[]>("view:groom:data", []);
  const [loading, setLoading] = useState<boolean>(images.length === 0);
  const [selectedCategory, setSelectedCategory] = useClientMemoryState("view:groom:category", "all");
