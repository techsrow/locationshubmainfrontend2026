    ...
    const restorePosition = (entryKey: string) => {
      ...
      const attempt = () => {
        ...
        if (stableFrames >= 4 || frame >= 180) {
          ...
          // Some sections/reveal libraries initialise in a hidden state and
          // only update from scroll/resize listeners. Back navigation can
          // restore the viewport without a fresh user scroll, so wake those
          // listeners once the DOM has reached the restored position.
          window.dispatchEvent(new Event("scroll"));
          window.dispatchEvent(new Event("resize"));
          window.dispatchEvent(
            new CustomEvent("locationshub:scroll-restored", {
