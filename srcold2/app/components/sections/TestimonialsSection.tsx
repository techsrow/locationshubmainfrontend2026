...
export default function TestimonialsSection() {
  const cachedTestimonials =
    getClientMemoryCache<Testimonial[]>("home:testimonials") ?? [];
  const [testimonials, setTestimonials] = useState<Testimonial[]>(cachedTestimonials);
  const [loading, setLoading] = useState(cachedTestimonials.length === 0);
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        setClientMemoryCache("home:testimonials", sorted);
