...
export default function OurSetsSection() {
  const cachedSets = getClientMemoryCache<SetItem[]>("home:sets") ?? [];
  const [sets, setSets] = useState<SetItem[]>(cachedSets);
  const [showAll, setShowAll] = useState(
    () => getClientMemoryCache<boolean>("home:sets:showAll") ?? false
  );
  const [loading, setLoading] = useState(cachedSets.length === 0);
  ...
  useEffect(() => {
    const fetchSets = async () => {
      try {
        ...
        setClientMemoryCache("home:sets", sorted);
        setSets(sorted);
      } catch (error) {
    ...
    {set.title}
      ...
      {!showAll && sets.length > 6 && (
        <div className="text-center mt-10">
          <button
            onClick={() => {
              setClientMemoryCache("home:sets:showAll", true);
              setShowAll(true);
            }}
