...
export default function BookingCalendar({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        console.log("Loading product:", `/products/${slug}`);
  ...
  useEffect(() => {
    ...
    const loadLockedDates = async () => {
      try {
        const res = await api.get<any>("/bookings/locked-dates");

        const formattedLockedDates = (res.data?.dates || []).map((item: any) =>
          format(new Date(item.date), "dd-MM-yyyy"),
        );

        setLockedDates(formattedLockedDates);
      } catch (error) {
  ...
  setBooking({
    ...
    bookingAmount: Number(product.bookingAmount),
    date: format(selectedDate, "yyyy-MM-dd"),
    slots: [selectedSlot], // keep array for compatibility
  });
  ...
  return (
    <div className="mx-auto booking-calendar">
      ...
      <div className="mt-6">
        <h3 className="font-semibold mb-3">Select your slot *</h3>
